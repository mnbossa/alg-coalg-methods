...
** Composite map \( c := ^\vartriangleright \circ ^\vartriangleleft : P \rightarrow P \) is a closure operator 
...
** Composite map \( k := ^\vartriangleleft \circ ^\vartriangleright : Q \rightarrow Q \) is an *interior* operator 
...
** Given the set of concepts \(\mathfrak{B}(G,M,R)\) and the natural projections \( \pi_1: \mathfrak{B}(G,M,R) \to \powerset(G) \) and \( \pi_2: \mathfrak{B}(G,M,R) \to \powerset(M)^\partial \), let be:
\[
...
then \( \vartriangleright \circ \pi_1 = \pi_2 \) and \( \vartriangleleft \circ \pi_2 = \pi_1 \) are order-isomorphisms. 
...
\]
are concepts. Furthermore, \( (g,m) \in R \iff \gamma(g) \subseteq \mu(m) \)
